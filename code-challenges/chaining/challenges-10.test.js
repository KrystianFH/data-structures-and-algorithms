...
const createServer = () => {
  ...
  var server = app.listen(3301, function () {
    ...
    function about (request, response){
      response.send('stuff about me');
    }
    function foods (request, response){
      let arrFoods = ['food1', 'food2', 'food3'];
      response.send(arrFoods);
    }

  app.get('/hello', hello);
  app.get('/aboutme', about);
  app.get('/favoritefoods', foods);
...
const count = (target, input) => {
  ...
  input.map(searchInsideArray => {
    searchInsideArray.map(element =>{
      if(element === target){
        instanceCounter++;
      }
    });
...
const totalSum = (input) => {
  ...
  return input.reduce((accum, val) => {
    return accum + val.reduce((innerAccum, innerVal) =>
    innerAccum + innerVal, 0);
  }, 0);
};
